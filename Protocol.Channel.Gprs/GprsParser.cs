...
namespace Protocol.Channel.Gprs
{
    public class GprsParser : IGprs
    {
        ...
        public bool DSStopService()
        {
            ...
            //InvokeMessage(String.Format("调用dll关闭端口{0}...", port), "      ");
            if (DTUdll.Instance.Started)
                stoped = DTUdll.Instance.StopService();

            //InvokeMessage(String.Format("调用dll关闭端口结束{0}...", port), "      ");
            if (SerialPortStateChanged != null)
                SerialPortStateChanged(this, new CEventSingleArgs<CSerialPortState>(new CSerialPortState()
                {
                    PortType = this.m_portType,
        ...
        public bool SendTru(uint dtuID, string truMsg)
        {
            try
            {
                byte[] byteArray = System.Text.Encoding.Default.GetBytes("TRU" + truMsg);
                byte[] bts = new byte[] { 84, 82, 85, 13, 10 };
                SendHex(dtuID, byteArray);
                return true;
            }
            catch (Exception)
            {
        ...
        public void Close()
        {
            ...
            catch (ThreadAbortException tae)
            {
                ...
                InvokeMessage("线程终止异常", tae.Message);
            }
            catch (Exception exp)
            {
                    ...
                    if (msg.Contains("1K"))
                    {
                        ...
                        if (Down.Parse_Flash(msg, EChannelType.GPRS, out batch))
                        {
                            if (this.BatchDataReceived != null)
                                this.BatchDataReceived.Invoke(null, new BatchEventArgs() { Value = batch, RawData = msg });
                        }
                    }
                    #endregion
                        ...
                        else
                        {
                            ...
                            if (downReport != null)
                            {
                                ...
                                if (this.DownDataReceived != null)
                                    this.DownDataReceived.Invoke(null, new DownEventArgs() { Value = downReport, RawData = rawData });
                            }
                        }
                        #endregion
                    }
                ...
                else if (msg.Contains("BEG"))
                {
                    ...
                    if (Down.Parse_SD(msg, sid, out sd))
                    {
                        ...
                        if (this.BatchSDDataReceived != null)
                            this.BatchSDDataReceived.Invoke(null, new BatchSDEventArgs() { Value = sd, RawData = msg });
                    }
                }
            ...
            while (true)
            {
                ...
                for (int i = 0; i < dataListTmp.Count; ++i)
                {
                    try
                    {
                        ...
                        //1119
                        //Manager.WriteToFileClass writeClass = new Manager.WriteToFileClass("ReceivedLog");
                        //Thread t = new Thread(new ParameterizedThreadStart(writeClass.WriteInfoToFile));
                        //t.Start("GPRS： " + "长度：" + data.Length + " " + data + "\r\n");
                        string gprs = dat.m_modemId.ToString("X").PadLeft(8, '0');
                        ...
                        if (data.Contains("$"))
                        {
                            if (data.Contains("$TS"))
                            {
                                ...
                                data = data.Trim();
                                data = data + "TS";
                                if (this.ParseData(data, gprs, dat.m_modemId))
                                {
                            ...
                            {
                                if (data.Contains("1G21") || data.Contains("1G22") || data.Contains("1G25"))
                                {
                                    // 发送回执
                                    SendTru(dat.m_modemId);
                                }
                                if (data.Contains("1G29"))
                                {
                                    int indexStart = data.IndexOf("$");
