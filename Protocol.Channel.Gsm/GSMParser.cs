    ...
    public class GsmParser : IGsm
    {
        ...
        public bool InitGsm()
        {
            try
            {
                ...
                //for (int k = 1; k <= 40; k++)
                //{
                //    InvokeMessage("SendCmpd" + k.ToString(),"GSM发送");
                ...
                InvokeMessage("SendSetCSMP", "GSM发送");
                SendSetCSMP(out isSendSuccess);
                if (!isSendSuccess) return false;
                InvokeMessage("SendQueryCSMP", "GSM发送");
                SendQueryCSMP(out isSendSuccess);
                if (!isSendSuccess) return false;
                InvokeMessage("SendSetCNMI", "GSM发送");
                SendSetCNMI(out isSendSuccess);
                if (!isSendSuccess) return false;
                InvokeMessage("SendQueryCNMI", "GSM发送");
                SendQueryCNMI(out isSendSuccess);
                if (!isSendSuccess) return false;
                InvokeMessage("SendSave", "GSM发送");
